    ...
    if not user:
        ...
        try:
            ...
            if member.status in ["member", "administrator", "creator"]:
                ...
                await msg.answer(f"✅ Оплачено до {user['expire_date'].strftime('%d.%m.%Y')}.\nВаша ссылка: {link.invite_link}")
        except: await msg.answer("Ошибка проверки статуса.")
...
async def cb_decision(callback: types.CallbackQuery):
    ...
    if action == "app":
        ...
        await bot.send_message(uid, f"✅ Одобрено! Ваша ссылка:\n{link.invite_link}")
