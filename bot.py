...
async def cmd_clear_db(message: types.Message):
    ...
    btn = InlineKeyboardButton(text="🧨 Очистить / Clear", callback_data="conf_clear")
    kb = InlineKeyboardMarkup(inline_keyboard=[[btn]])
    await message.answer("🧨 Очистить базу данных? / Clear DB?", reply_markup=kb)
...
async def cmd_stats(message: types.Message):
    ...
    if not users:
        await message.answer("База пуста. / DB empty.")
        return
    for u in users:
        exp = u.get("expire_date")
        date_s = exp.strftime('%d.%m.%Y') if exp else "Ожидает / Waiting"
        text = f"👤 {u.get('full_name', 'User')}\nID: `{u['user_id']}`\n📅 До: {date_s}"
        btn = InlineKeyboardButton(text="❌ Удалить / Kick", callback_data=f"kick_{u['user_id']}")
        kb = InlineKeyboardMarkup(inline_keyboard=[[btn]])
...
async def cmd_start(message: types.Message):
    if message.from_user.id == CFG.admin_id:
        b1 = InlineKeyboardButton(text="📊 Статистика / Stats", callback_data="admin_stats_cmd")
        b2 = InlineKeyboardButton(text="🧨 Очистить базу / Clear DB", callback_data="admin_clear_cmd")
        kb = InlineKeyboardMarkup(inline_keyboard=[[b1], [b2]])
        await message.answer("🛠 Админ-панель / Admin panel:", reply_markup=kb)
        return
    b1 = InlineKeyboardButton(text="💳 Оплатить / Pay", callback_data="pay")
    b2 = InlineKeyboardButton(text="🔎 Моя подписка / My sub", callback_data="check_my_sub")
    kb = InlineKeyboardMarkup(inline_keyboard=[[b1], [b2]])
...
async def handle_receipt(message: types.Message):
    if message.from_user.id == CFG.admin_id: return
    b1 = InlineKeyboardButton(text="✅ Одобрить", callback_data=f"app_{message.from_user.id}")
    b2 = InlineKeyboardButton(text="❌ Отклонить", callback_data=f"ref_{message.from_user.id}")
    kb = InlineKeyboardMarkup(inline_keyboard=[[b1], [b2]])
