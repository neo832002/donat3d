...
async def check_expirations_test():
    while True:
        try:
            now = datetime.now()
            # Проверяем только активных (кто уже зашел)
...
async def on_user_join(event: ChatMemberUpdated):
    ...
    if user_data:
        ...
        await subs_collection.update_one(
            {"user_id": user_id},
            {"$set": {"status": "active", "expire_date": expire}}
        )
        try: await bot.send_message(user_id, f"✅ Доступ активирован! Срок 1 минута начался.\nExpires at: {expire.strftime('%H:%M:%S')}")
        except: pass
...
async def cmd_start(message: types.Message):
    ...
    await message.answer("👋 Пришлите фото чека. Подписка активируется после входа в канал.", reply_markup=kb)
...
async def admin_decision(callback: types.CallbackQuery):
    ...
    action, uid = callback.data.split("_"), int(callback.data.split("_")[1])
    ...
    if action == "ok":
        ...
        await subs_collection.update_one(
            ...
            {"$set": {
                ...
                "expire_date": datetime.now() + timedelta(days=999) # Пока не зашел
            }}, upsert=True
        )
        await bot.send_message(uid, f"✅ Оплата одобрена!\n\nВступите в канал по ссылке (срок пойдет после входа):\n{link.invite_link}")
    else:
        await bot.send_message(uid, "❌ Отказано в доступе. / Access denied.")
...
async def main():
    ...
    log.info("Bot started. Sub starts strictly after JOIN.")
